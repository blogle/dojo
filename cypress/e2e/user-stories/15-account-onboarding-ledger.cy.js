...
describe("User Story 15 — Account Onboarding Ledger Integrity", () => {
  ...
  const verifyOpeningBalanceRow = (account) => {
    cy.contains("#transactions-body tr", account.name, { timeout: 20000 }).within(() => {
  ...
  it("records opening balances, shows cards/modals, and reconciles hero stats", () => {
    ...
    accountPage.verifyAccountBalance(checkingAccount.name, checkingAccount.heroDisplay);
    accountPage.verifyAccountBalance(creditAccount.name, creditAccount.heroDisplay);
