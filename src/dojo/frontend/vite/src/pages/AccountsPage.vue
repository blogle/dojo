...
const institutionList = [
    "Chase", "Bank of America", "Wells Fargo", "Citigroup", "American Express", 
    "Vanguard", "Fidelity", "Charles Schwab", "Capital One", "Discover", "USAA", 
    "Navy Federal Credit Union", "PNC Bank", "TD Bank", "Goldman Sachs", 
    "Morgan Stanley", "Interactive Brokers", "E*TRADE", "Robinhood", "Coinbase", 
    "SoFi", "Ally Bank"
];
...
const WIZARD_SCHEMAS = {
    cash: {
        ...
        fields: [
            { id: "institution_name", label: "Institution", type: "text", list: "institutions", placeholder: "Select or type..." },
            { id: "account_name", label: "Account Nickname", type: "text", placeholder: "e.g. Primary Checking" },
            { id: "interest_rate_apy", label: "Interest Rate (APY %)", type: "number", step: "0.01", placeholder: "0.00" }
        ]
    },
    credit: {
        label: "Credit Card",
        ...
        balanceHelper: "Enter as a positive number. The system will track it as a liability.",
        account_class: "credit",
        account_type: "liability",
        account_role: "on_budget",
        fields: [
            { id: "institution_name", label: "Institution", type: "text", list: "institutions", placeholder: "Select or type..." },
            { id: "account_name", label: "Card Nickname", type: "text", placeholder: "e.g. Gold Card" },
            { id: "card_type", label: "Network", type: "select", options: ["Visa", "Mastercard", "American Express", "Discover"] },
            { id: "apr", label: "Purchase APR (%)", type: "number", step: "0.01" },
            { id: "cash_advance_apr", label: "Cash Advance APR (%)", type: "number", step: "0.01" }
        ]
    },
    accessible: {
        label: "Savings / CD",
        ...
        fields: [
            ...
            { id: "interest_rate_apy", label: "APY (%)", type: "number", step: "0.01" },
            { id: "early_withdrawal_penalty", label: "Early Withdrawal Penalty", type: "checkbox", helper: "Check if this account charges fees for early access." }
        ]
    },
    investment: {
        label: "Investment",
        ...
        balanceHelper: "Holdings (Stocks/ETFs) are tracked separately via the Positions table.",
        ...
        fields: [
            { id: "institution_name", label: "Institution", type: "text", list: "institutions", placeholder: "Select or type..." },
            { id: "account_name", label: "Account Nickname", type: "text" },
            { id: "tax_classification", label: "Tax Classification", type: "select", options: ["Taxable Brokerage", "Traditional IRA", "Roth IRA", "401k", "403b", "529"] },
            { id: "is_self_directed", label: "Self-Directed", type: "checkbox", helper: "Check if you manage trades manually." },
            { id: "risk_free_sweep_rate", label: "Sweep Fund Rate (%)", type: "number", step: "0.01" }
        ]
    },
    loan: {
        label: "Loan / Mortgage",
        ...
        fields: [
            { id: "institution_name", label: "Institution", type: "text", list: "institutions", placeholder: "Select or type..." },
            { id: "loan_name", label: "Loan Nickname", type: "text", placeholder: "e.g. Student Loan A" },
            { id: "initial_principal_minor", label: "Original Principal", type: "number", step: "0.01" }, // Logic to x100
            { id: "interest_rate_apy", label: "Interest Rate (%)", type: "number", step: "0.01" },
            { id: "mortgage_escrow_details", label: "Escrow Notes", type: "text", placeholder: "Optional tax/insurance details" }
        ]
    },
    tangible: {
        ...
        account_role: "tracking",
        fields: [
            { id: "asset_name", label: "Asset Name", type: "text", placeholder: "e.g. 2018 Honda Civic" },
            { id: "acquisition_cost_minor", label: "Original Cost Basis", type: "number", step: "0.01" } // Logic to x100
        ]
    }
};

const currentSchema = computed(() => wizardType.value ? WIZARD_SCHEMAS[wizardType.value] : null);

const wizardStepTitle = computed(() => {
    switch (wizardStep.value) {
        case 1: return "Select Account Type";
        case 2: return currentSchema.value?.title || "Account Details";
        case 3: return "Opening Balance";
        case 4: return "Review Configuration";
        default: return "";
    }
});
...
const canProceed = computed(() => {
    ...
    if (wizardStep.value === 2) {
        ...
        if (wizardType.value !== 'loan' && wizardType.value !== 'tangible' && !wizardData.account_name) return false;
        return true;
    }
...
const formatReviewValue = (field, value) => {
    ...
    if (field.id.includes('minor')) { // Check if it's a minor unit field in schema
         return value ? formatCurrency(value) : "—"; // Wait, value is user input (float), formatCurrency expects cents? No, helper formatAmount expects cents.
    }
    return value || "—";
};
...
const handleCreateAccount = async () => {
    ...
    const name = wizardData.account_name || wizardData.loan_name || wizardData.asset_name;
	...
	const payload = {
        ...
        // Extra fields
        interest_rate_apy: wizardData.interest_rate_apy ? Number(wizardData.interest_rate_apy) : null,
        card_type: wizardData.card_type,
        apr: wizardData.apr ? Number(wizardData.apr) : null,
        cash_advance_apr: wizardData.cash_advance_apr ? Number(wizardData.cash_advance_apr) : null,
        term_end_date: wizardData.term_end_date || null,
        early_withdrawal_penalty: !!wizardData.early_withdrawal_penalty,
        risk_free_sweep_rate: wizardData.risk_free_sweep_rate ? Number(wizardData.risk_free_sweep_rate) : null,
        is_self_directed: !!wizardData.is_self_directed,
        tax_classification: wizardData.tax_classification,
        initial_principal_minor: wizardData.initial_principal_minor ? Math.round(Number(wizardData.initial_principal_minor) * 100) : null,
        mortgage_escrow_details: wizardData.mortgage_escrow_details,
        asset_name: wizardData.asset_name,
        acquisition_cost_minor: wizardData.acquisition_cost_minor ? Math.round(Number(wizardData.acquisition_cost_minor) * 100) : null,
	};
