import { accountGroupDefinitions, accountTypeMapping, selectors } from "../../constants.js";
  ...
  try {
    ...
    if (balanceMinor !== 0) {
      try {
        ...
        alert("Account created, but we couldn't record the opening balance transaction. You can retry from the ledger later.");
      }
    }
...
const initAccountFilter = () => {
  ...
  const updateSectionActive = (active) => {
    sectionLinks.forEach((link) => {
      ...
      link.classList.toggle("accounts-page__filter-button--active", section === active);
    });
  };
