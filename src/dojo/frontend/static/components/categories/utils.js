...
export const filterUserFacingCategories = (categories = []) =>
  categories.filter((category) => category && category.is_active !== false && !systemCategoryIds.has(category.category_id));
...
export const findBudgetCategory = (categoryId, { includeHidden = false } = {}) => {
  if (!categoryId) {
    return null;
    }
  const state = store.getState();
