...
export const makeRowEditable = (row, data, config) => {
  ...
  config.forEach(col => {
    ...
    if (col.type === "money") {
        ...
        if (col.attrs) {
             Object.entries(col.attrs).forEach(([k, v]) => input.setAttribute(k, v));
        }
        ...
        // Populate
        if (col.options) {
            populateSelect(select, col.options, { valueKey: col.valueKey || "value", labelKey: col.labelKey || "label" }, col.placeholder);
        }
        select.value = data[col.key] || "";
        
        if (col.attrs) {
             Object.entries(col.attrs).forEach(([k, v]) => select.setAttribute(k, v));
        }
