...
class NetWorthResponse(BaseModel):
    ...
    liabilities_minor: int = Field(
        description="Sum of active liability account balances."
    )
    ...
    net_worth_minor: int = Field(
        description="Assets - liabilities + positions (minor units)."
    )
    net_worth_decimal: Decimal = Field(
        description="Net worth expressed in whole units (Decimal)."
    )
