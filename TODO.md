- **Title:** Research Visual Regression Testing for Frontend
- **Priority:** P3
- **Effort:** Medium
- **Trigger:** Identified during a bug fix where UI layout issues (e.g., element overflow, unwanted scrollbars) were not caught by existing E2E tests.
- **Why defer:** Not blocking critical functionality; requires research into tools and integration.
- **Proposed next step:** Investigate integrating a visual regression testing tool (e.g., Percy, Applitools, or a Cypress plugin) into the frontend test suite. Evaluate tools based on ease of integration, maintenance, and ability to catch subtle UI regressions.
- **Acceptance Criteria:** A recommendation for a visual regression testing tool or a decision to defer based on cost/benefit analysis. Initial tests implemented for key UI components.
- **Links:** None (this is a new item)
