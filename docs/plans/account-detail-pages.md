...
- [ ] (YYYY-MM-DD HH:MMZ) Add backend endpoints for account details + per-account transactions + per-account balance history (including `status` filtering and balance continuity guardrails).
...
- [ ] (YYYY-MM-DD HH:MMZ) Add automated tests for new API endpoints and run the relevant test suite.
