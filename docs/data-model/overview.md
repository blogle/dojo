...
| `account_reconciliations`          | Stores point-in-time confirmations of account balances against external statements.    | One row per reconciliation event.               | `reconciliation_id` (UUID) | `account_id` -> `accounts`                        | Acts as a "commit" of the ledger state.                                                                                                            |
